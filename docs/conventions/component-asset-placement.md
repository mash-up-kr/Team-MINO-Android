...
판정이 "Figma에 컴포넌트로 존재하는가"에 달려 있으므로 **노드를 열어 확인한다.** 눈으로 본 생김새나 이름으로 판단하지 않는다.

- **컴포넌트셋의 인스턴스면** 디자인 시스템 자산이다. 자기 속성 축(size·variant 등)을 갖는다.
- **로컬 프레임이면** 아니다. 사각형·텍스트 같은 도형을 그 화면에서 직접 조립한 것이다.

> [!WARNING]
> **로컬 프레임이 컴포넌트 인스턴스를 품고 있는 경우를 조심한다.** 안에 버튼·아이콘 인스턴스가 들어 있어도 **바깥 프레임 자체가 컴포넌트가 아니면 디자인 시스템 자산이 아니다.** 부품이 디자인 시스템 것이라는 사실과 조립물이 디자인 시스템 것이라는 주장은 다르다.

**정의 노드가 안 열리는 것은 판정 근거가 아니다.** 화면 파일에서 보이는 컴포넌트 ID는 인스턴스의 내부 참조라 그대로는 열리지 않는다([figma-design-fidelity §1.1](figma-design-fidelity.md#11-클라우드의-제약--컴포넌트-정의-노드)). **열리지 않았다고 해서 "컴포넌트가 아니다"로 읽으면 안 된다** — 그 반대에 가깝다. 확인이 필요하면 사용자에게 링크를 받는다.
