...
> [!NOTE]
> 아래 "컴포넌트 토큰 층"의 *"슬롯 → 디자인 토큰 키 매핑만 담는다"* 조항은 모서리·치수에 한해 [모서리(셰이프)는 토큰 foundation을 두지 않고 컴포넌트 토큰이 실측값을 직접 든다](2026-08-13-no-shape-token-foundation.md)로 좁혀졌다. 색·타이포·그림자에 대해서는 이 ADR이 그대로 유효하다.
