...
- 형식은 Nygard 스타일: **상태 · 날짜 · 작성자 · 맥락 · 결정 · 고려한 대안 · 결과**
- **작성자**는 문서를 쓴 사람이 아니라 **이 결정이 누구의 작업과 관련되면 좋을지**를 남기는 필드다 (담당자·관련 작업자). 대화 중 자동 생성 시에도 이 관점으로 채운다.
- **고려한 대안**은 채택하지 않은 선택지와 배제 이유를 남기는 필드다. "왜 이 결정을 했는가"만큼 "왜 다른 방법이 아니었는가"를 남기는 게 핵심이다 — 이게 없으면 다음 세션에서 이미 배제된 대안이 처음부터 다시 제안·논의되어 ADR을 쓰는 목적(재논의 방지)이 무색해진다. 대화에 언급되지 않았다면 추측해서 채우지 말고 사용자에게 먼저 확인한다.
- 언어는 한국어
