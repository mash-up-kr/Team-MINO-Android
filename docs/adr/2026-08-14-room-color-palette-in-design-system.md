...
**가시성 제약이 위치를 결정한다 — 이것이 1순위 근거다.** 팔레트 값에 닿을 수 있는 곳이 디자인 시스템 안뿐이다. 이걸 우회하려면 `AtomicColorToken`을 public으로 열어야 하는데, 그러면 README §4.3이 정한 "외부는 홀더 프로퍼티로만 접근한다"가 무너지고 원시값이 feature로 새어 나간다. 내부 리팩터가 외부로 새지 않게 하려고 만든 캡슐화를 팔레트 하나 때문에 깨는 것은 비용이 맞지 않는다. **아래 두 근거는 이 제약 위에 얹히는 보조 근거이며, 단독으로는 이 배치를 정당화하지 않는다.**

**소비자가 여럿이라 SSOT가 필요하다.** 방 목록 카드·지도 마커·뱃지가 각자 같은 12색을 들면, 디자이너가 색 하나를 바꿨을 때 고쳐야 할 자리를 코드가 알려주지 못한다. enum 하나면 `when`이 망라적이라 항목 추가·변경이 컴파일 타임에 드러난다. **다만 이 근거만으로 `:core:design-system` 배치가 정당화되지는 않는다** — SSOT는 `:core:common:ui`에 두어도 똑같이 만족된다. 소비자가 여럿이라는 사실은 "공용 모듈에 둔다"까지만 말하고, **어느** 공용 모듈이냐는 위 가시성 제약이 정한다.
...
**이 ADR의 적용 범위 — 토큰 값에 직접 닿는 컴포넌트에 한정한다.** `AtomicColorToken`처럼 `internal`인 토큰을 직접 참조해야 해서 **다른 모듈에서는 만들 방법 자체가 없는** 컴포넌트만 이 선례에 해당한다. 홀더 프로퍼티(`MinoAndroidTheme.*`)만 쓰는 공용 합성 컴포넌트는 여러 feature가 쓰더라도 이 ADR을 근거로 `:core:design-system`에 두지 않는다. 그 판정과 승격 경로는 [`conventions/component-asset-placement.md`](../conventions/component-asset-placement.md)가 소유한다.
