...
- 서버 문자열과 도메인 값의 대응은 `RoomMapper`만 안다([`core/domain/README.md`](../../../core/domain/README.md) §5).
...
- `description`은 nullable이 아니다. 서버의 `null`은 `RoomMapper`가 빈 문자열로 흡수한다 — 기존 `Room`과 같은 규칙이다.
- `placeCount`는 0 이상이다. 이 값은 방 전체의 장소 수일 뿐 **이번 공유의 중복 여부를 뜻하지 않는다**(spec §4 가정).
- `thumbnailImageUrls`가 4장을 넘으면 앞 4장만 쓴다. 판정은 `RoomMapper`가 한다.

### 1.3 `SharedPlaceSaveRequest`

한 번의 공유에서 확정된 저장 실행 단위. spec §2.3의 「저장 선택(Save Selection)」에 대응한다.
...
```
suspend fun requestSave(request: SharedPlaceSaveRequest)
```

- 반환값이 없다. 서버가 202로 접수만 확정하고 결과는 비동기로 갈린다([contracts/shared-place-save-api.md](./contracts/shared-place-save-api.md)).
- 실패는 `MinoDomainException.Network` / `.Http`로 던진다. 재시도 판정은 호출자(`SharedPlaceSaveWorker`)가 소유한다([research.md R-005](./research.md)).
...
- 저장 요청은 UseCase를 두지 않는다. `SharedPlaceSaveWorker`가 `SharedPlaceRepository`를 직접 호출하며, 그 사이에 판단이 없다.
...
| `SharedPlaceSaveRequestBody` | 요청 | [contracts/shared-place-save-api.md](./contracts/shared-place-save-api.md) |
...
- 응답에 서버가 필드를 더해도 `ignoreUnknownKeys = true`가 흡수한다.
...
| `Save` | `[저장하기]` 탭 | 워커 예약 → `SideEffect.SavedAndFinish` |
