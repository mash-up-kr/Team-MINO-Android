...
- **spec이 저장 탭을 귀착지로 못박는다** *(spec 5.0.0에서 홈 진입만 예외가 되었다 — 아래 정정 참고)*: TS-007(알림 진입)·TS-037(홈 진입) 모두 [나가기]가 "알림 탭이 아니라"·"홈 탭이 아니라" **A방의 방 상세 `Half`로** 가라고 규정한다. 어느 진입점으로 들어와도 사용자가 남는 자리는 저장 탭이다. 편입 구조에서 이것은 `selectedPinId = null` 한 줄이다 — 방 상세 시트가 그 아래에 그대로 살아 있기 때문이다.
  - **(spec 5.0.0 정정)** PRD 13.0.0이 [SCR-006] Flow A에 홈 예외를 새기면서 TS-037이 뒤집혔다 — 홈 카드로 들어와 방을 바꾸지 않았다면 [나가기]는 홈 탭으로 되돌린다. **편입 결정 자체는 흔들리지 않는다.** 이 근거는 편입을 뒷받침한 여섯 갈래 중 하나였고, 나머지(지도 한 벌·마커 전이·컨트롤 한 벌·방 전환이 상태 갱신)는 그대로다. 오히려 편입 구조에서 이 예외가 `returnsToHomeOnClose` 분기 하나로 끝난다 — 진입형 Activity였다면 `setResult`로 홈에 되돌릴 값을 실어 보내야 했다.
...
## D27. 카메라 write가 둘이면 사용자의 지목이 이긴다

- **Decision**: `RoomListViewModel` 안에서 `mapCenter`를 쓰는 자리를 **지목**(장소 상세를 여는 두 경로 — `OnPlaceSelected`·탭 간 요청 소비)과 **기본값**(탭 진입에 딸린 위치 해석 — `OnScreenEntered`·`OnLocationPermissionResult`)으로 가르고, `selectedPinId != null`인 동안에는 기본값 쪽 write를 건너뛴다. 사용자가 직접 누른 [현재 위치]는 지목이므로 막지 않는다.
- **Rationale**: [D25](#d25-지도와-지도-위-컨트롤을-한-벌로-합친다)가 "카메라를 실제로 움직이는 주체"를 `RoomListViewModel` 하나로 모았지만, **그 하나 안에서 write가 여럿이라는 것**과 그것들의 우선순위는 비워 두었다. 홈·알림 진입은 탭 전환(→ `OnScreenEntered`)과 장소 상세 열기가 같은 순간이라 두 write가 겹치고, 어느 쪽이 이기는지가 **위치 해석 속도에 달린다** — 캐시된 마지막 위치가 있으면 즉시 끝나 지목이 이기고, 없어 활성 측위로 넘어가면 최대 10초 뒤에 도착해 지목을 덮는다. 순서로는 고정되지 않으므로 규칙으로 고정한다.
- **왜 「지목이 이긴다」인가**: 진입에 딸린 이동은 사용자가 무엇을 보려는지 모를 때의 기본값이고, 선택 핀은 사용자가 방금 지목한 대상이다. 반대로 두면 사용자가 연 장소가 화면 밖으로 밀려나므로 FR-002가 성립하지 않는다.
- **감수하는 것**: 장소 상세가 열려 있는 동안에는 `mapCenter`가 고정되므로 `NEARBY` 마커 정렬의 기준점도 함께 고정된다. 그 기준점이 지금 화면에 보이는 자리와 어긋나지 않으므로 오히려 일관된다.
- **(spec 5.0.0에서 결정 — 실기기 검수 반영)**
...
| 카메라 write 사이의 우선순위 | **해소** | D27 — 지목(선택 핀)이 기본값(진입 시 위치 해석)을 이긴다. D25가 비워 둔 자리 |
