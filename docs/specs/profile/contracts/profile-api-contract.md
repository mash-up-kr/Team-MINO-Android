...
> **출처**: `https://api.gguk.org/api-docs-json` (Team MINO API 1.0.0) · **조회 시점**: 2026-08-28T17:13:03+09:00
>
> **plan 5.0.0의 재조회에서 세 오퍼레이션은 직전 조회(2026-08-28T11:44:31+09:00)와 동일했다.** 값 도메인이 안정됐다 — 아래 인용한 스키마를 그대로 근거로 쓸 수 있다.
>
> 그 이전에는 하루 만에 `avatar.color`가 자유 문자열(`minLength 1`·`maxLength 20`)에서 **13개 `enum`**으로 좁혀진 적이 있다(2026-08-27T22:15:17 → 2026-08-28T01:12:44). 이 문서를 근거로 삼기 전에 위 URL을 다시 조회해 대조하는 습관이 필요하다.
>
...
> 닉네임(공백 포함 한글/영문 2~15자)·아바타 수정

요청은 `POST`와 같은 속성(같은 13개 `enum` 포함)이되 `"required": []`다 — 모든 필드가 선택이다. **이 앱은 언제나 두 값을 함께 보낸다.** 화면이 닉네임과 아바타를 한 폼으로 다루고(FR-001), 부분 전송은 "아바타를 안 골랐다"와 "아바타를 건드리지 않았다"를 구분하지 못하기 때문이다. 응답 `200`·실패 `401`은 `GET`과 같다.

---

## 2. spec과 어긋나는 지점

**조용히 넘기지 않는다.** 6건 중 **4건이 닫혔고 2건이 남는다.** 4번은 plan 5.0.0 구현 뒤 T083의 기기 실측으로 닫혔다.
...
| 6 | 응답의 `avatar`가 `nullable: true`인데 요청에서는 `required` | 앱은 언제나 아바타를 보낸다(EC-002의 기본 아바타 포함) | `null`이면 기본 아바타로 읽는다. **언제 `null`이 되는지 문서에 없다** | [D37](../research.md#d37-아바타-식별자--도메인-profileavatar-enum-서버-표현은-avatarcolor-문자열) |
...
| 2 | 닉네임 `maxLength: 15` vs spec의 상한 없음 | **[spec 2.0.0](../spec.md) §5가 확정했다** — 상한을 두지 않고, 16자 이상은 서버가 거절해 저장 실패로 보인다(신설 EC-014). 알고 받아들이는 어긋남이라 협의 항목이 아니다 |
| 3 | 닉네임 `pattern`이 공백 허용 vs spec의 공백 불가 | **[spec 2.0.0](../spec.md) §5가 확정했다** — 공백 불가를 유지한다. 클라이언트가 더 좁아 서버까지 가지 않으므로 실패가 나지 않는다 |
...
- `enum`의 13번째 값 `gray`는 **보내지 않는다.** 방에서 `gray`는 "색을 고르지 않은 방"의 값인데 프로필에는 그 상태가 저장되지 않는다. 받는 쪽은 모르는 값·`null`과 같게 기본 아바타로 읽는다.
...
| 닉네임 규칙(spec 2.0.0에서 확정) | 다시 논의된다면 **spec부터** 고친다. 클라이언트 검증은 `ValidateNicknameUseCase` 한 곳 |
