...
4. **장소 액션 — 다른 방에 공유 / 삭제** (TS-007~009)
   - 장소 카드 [...] → [다른 방에 공유] → 방 선택 시트(`Full` 676dp 고정) → 방 선택 → [공유하기] → 실제로 `PlaceRepository.sharePlaces`(서버 `POST /pins/{pinId}/duplicate`, [research.md D14](./research.md))가 호출돼 대상 방에 장소가 복제되는지 확인 → `공유가 완료되었습니다.` 3초 토스트, 화면 유지 확인(UX-002). 이미 저장된 방은 체크+비활성 확인(EC-004), 겹치면 서버가 `409`로 전체 거절하는지 확인.
