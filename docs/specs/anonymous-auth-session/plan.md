...
이 계획의 범위는 `:core:domain`·`:core:data`·`:core:error-handling`과, FR-007이 요구하는 `:app`의 백업 규칙까지다. **세션 확보를 호출하는 코드는 범위 밖이다** — 진입 화면 구현은 spec §3.2가, 임시 배선을 두지 않기로 한 결정(plan 1.1.0)은 §전제와 이연 항목이 소유한다. 화면이 지켜야 할 조건은 계약으로만 명시한다.
...
**규모/범위**: 신규 파일 — 도메인 3개, 데이터 10개(원천 접근자 2쌍·예외 매핑·Ktor 플러그인·Repository·DI 3), 예외 리프 1개(기존 파일 수정), 테스트 5개(Fake 픽스처 2개 포함). 삭제 파일 13개 (R-013). 새로 그리는 화면 0개
...
| G-3 | **원칙 III — 결정 기록** | 되돌리기 어려운 결정이 기록되고 ADR 승격 대상이 식별되었는가 | PASS | PASS — 16건이 [research.md](./research.md)에 기록됐고, 그중 다른 feature를 구속하는 3건이 ADR로 승격됐다(plan 1.1.0) — [Firebase 익명 인증](../../adr/2026-08-22-firebase-anonymous-auth-session.md) · [매핑 지점](../../adr/2026-08-22-domain-exception-mapping-per-source.md) · [재시도 소유자](../../adr/2026-08-22-session-retry-owned-by-caller.md) |
...
| G-8 | **기술 표준 — 검증 장치의 한계** | "CI가 잡아 줄 것"을 전제하지 않는가 | PASS | PASS — 검증 최소선을 빌드·JVM 단위 테스트로 잡는다. 실기기 수동 절차는 [quickstart.md](./quickstart.md)가 계속 소유하되, 호출자가 없어 이번 범위에서는 실행되지 않는다(§전제와 이연 항목) |
...
이번 범위에는 `EnsureAnonymousSessionUseCase`를 호출하는 프로덕션 코드가 없다.

spec §4 가정("그 화면이 만들어지기 전까지는 앱의 실제 진입 지점이 임시로 같은 역할을 맡는다")은 `:feature:main`의 `MainActivity`에 최소 배선을 두는 것을 허용하지만, **1.0.0에서 범위에 넣었던 그 임시 배선을 만들지 않기로 했다**(2026-08-22 결정, plan 1.1.0). 검증 수단을 위해 제품 코드에 한시적 호출을 심는 대신, 그 자리를 진입 화면 스펙이 채우기를 기다린다.

따라서 이 계획의 산출물은 도메인 계약·데이터 구현·단위 테스트까지이고, 호출자가 지켜야 할 조건은 [contracts/anonymous-auth-repository.md](./contracts/anonymous-auth-repository.md) §4의 C-1~C-8로만 존재한다. 파급(실기기 검증 절차의 실행 주체 부재)은 §전제와 이연 항목이 소유한다.
...
| 항목 | 상태 | 발효 조건·소유자 |
|---|---|---|
| Mino 서버 host 판정의 실효 | `NetworkModule`은 이미 `BuildConfig.API_BASE_URL`을 쓰지만 그 값이 flavor 플레이스홀더(`https://qa-api.example.com/`)라 첨부 판정이 항상 불일치다. FR-008·FR-011·TS-013·TS-016·SC-005·SC-006은 **계약과 구조로만** 충족되며 실제 첨부는 일어나지 않는다 | `build-logic`의 `Flavor.apiBaseUrl`에 실서버 도메인이 들어가는 별도 과제([`core/data/README.md`](../../../core/data/README.md) §4 NOTE). spec §4 가정이 그 시점을 서버 전환과 묶는다. 근거는 R-009 (서술은 plan 1.1.0에서 실제 코드에 맞춰 정정) |
...
| 세션 확보의 호출자 | 임시 배선을 두지 않기로 했으므로(plan 1.1.0, §세션 확보의 호출자) `ensureSession()`을 부르는 프로덕션 코드가 없다. [quickstart.md](./quickstart.md) §3의 실기기 검증을 실행할 주체가 없으며, 이번 범위의 판정선은 §2 자동 검증까지다 | 진입 화면 스펙 (PRD [SCR-001]) |
| 진입 화면 표현의 구현 | 화면 표현(FR-005·FR-018·FR-019·UX-001~004)과 재시도·지연 정책은 이번 범위에서 구현되지 않는다. 호출자가 지켜야 할 조건은 계약 C-1~C-8이 소유한다 | 같음 |
