...
| N-3 | **Mino host 판정 — 전제 해소됨** — FR-008 · FR-011 · TS-013 · TS-016 · SC-005 · SC-006 | plan §전제와 이연 항목이 이연했던 `Flavor.apiBaseUrl` 실서버 도메인 교체가 완료돼(qa·prod 모두 `https://api.gguk.org/`) 첨부 판정이 발효한다. 남은 공백은 그 host로 요청을 보내는 `Service`·호출부가 없다는 것이며, 이는 N-1과 같은 소관이다 |
