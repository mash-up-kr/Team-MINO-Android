...
- `RoomFormLauncher`([contracts/navigation-launchers.md](./contracts/navigation-launchers.md))가 최소 스텁으로라도 바인딩돼 있다(그렇지 않으면 Hilt 그래프 조립이 실패해 앱이 뜨지 않는다 — [research.md D6](./research.md)). 방 상세([SCR-005])는 2.0.0부터 `:feature:room` 내부 nested Route라 별도 Launcher 바인딩이 필요 없다([research.md D13](./research.md)).
...
4. **`Full` 방 목록** (TS-006~008, EC-003)
   ...
   - 방 카드 선택 → `navController.navigate(RoomDetailMain(roomId))` 호출 확인(방 상세로 전환, [research.md D13](./research.md)).
...
5. **공동방 생성** (TS-009, EC-004)
   ...
   - 생성 완료 → 방 상세로 직행하는지 확인(`RoomFormLauncher` 결과 → `RoomDetailMain` 내부 Route 전환 체이닝).
...
7. **방 상세 복귀 상태 유지** (EC-007) — `room-detail`([issue #161](../room-detail/spec.md), [plan.md](../room-detail/plan.md))이 구현된 이후에만 검증 가능
   - 방 상세를 `Full`로 본 뒤 `[X]` → 방 리스트도 `Full`로 복귀하는지 확인. 2.0.0부터 이 복귀는 별도 상태 전달 계약이 아니라 `RoomListMain`이 백스택에 남아 NavHost가 상태를 보존한 결과다([research.md D13](./research.md)).

## 기대 결과

- 위 1~6번은 `:feature:room` 단독 구현만으로 검증 가능하다(`RoomFormLauncher`는 스텁이어도 호출 여부까지만 확인. 방 상세 전환은 같은 모듈 내부 Route라 스텁이 필요 없다).
- 7번은 room-detail 구현이 선행돼야 실제 값 검증이 가능하다.
