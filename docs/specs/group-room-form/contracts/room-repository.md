...
- 허용 문자는 **한글·영문·숫자·공백**이다(FR-004).
- `[TBD]` **자모 단독(`ㄱ`·`ㅏ`)을 허용하는가.** spec FR-004·EC-005는 자모를 다루지 않는다. 허용하지 않으면 IME 조합 중인 글자가 입력 도중 오류로 번쩍이고, 허용하면 방 이름 `ㄱㄱㄱ`이 저장된다. 표시 억제와 저장 허용은 서로 다른 문제라 설계가 임의로 정하지 않는다 — spec 개정 대상이다.
- **길이를 판정하지 않는다.** 상한은 입력 차단이라 이 함수에 도달하는 값이 이미 15자 이하다([research.md](../research.md) R-009).
- `suspend`가 아니다. 글자 단위 입력마다 동기로 불린다(SC-002).

**단위 테스트 대상**: 위 표의 네 줄 + 숫자만·영문만·경계값(1자·15자).
