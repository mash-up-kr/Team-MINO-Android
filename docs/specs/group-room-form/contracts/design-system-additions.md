...
fun MinoTopNavigation(
    ...
    onNavigateBack: (() -> Unit)? = null,     // null이면 좌측 비움
    actionLabel: String? = null,              // null이면 우측 비움
    onActionClick: () -> Unit = {},
    colors: MinoTopNavigationColors = MinoTopNavigationDefaults.colors(),
)
```
...
| 방 썸네일 이미지 13종 | `:feature:roomform/src/main/res/drawable-{mdpi,xhdpi,xxhdpi}/` (WebP) | 실체가 이미지 에셋이고, 디자인 시스템은 이미지 에셋을 받지 않는다 ([R-008](../research.md) · [배치 규약 §1.1](../../../conventions/component-asset-placement.md#11-이미지-에셋)) |
...
`RoomColor` → drawable 매핑은 `:feature:roomform`의 `form/model/`이 소유한다. 도메인 값과 UI 에셋을 잇는 자리는 feature다([방 색상 팔레트 ADR](../../../adr/2026-08-14-room-color-palette-in-design-system.md) §결과).
