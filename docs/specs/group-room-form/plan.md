...
**참조 API 문서**: [Team MINO API](https://api.gguk.org/api-docs-json) `1.0.0` · **2026-08-28T11:39:53+09:00 조회 기준.** 이 계획의 서버 계약 판단은 그 시점의 문서를 근거로 하며, 인용된 스키마 제약의 소유자는 [contracts/room-api.md](./contracts/room-api.md) §1이다.
...
> [!NOTE]
> **모듈이 없다는 뜻이 아니다.** 2026-08-28 현재 `:feature:home`·`:feature:profile`·`:feature:main`·`:feature:sharereceiver`가 저장소에 있다. 그러나 아래 표의 **도착점 역할을 하는 화면은 그중 하나도 없다** — `:feature:home`은 아직 Sample 버튼만 있는 골격이고, 폼을 여는 유일한 코드는 `MainActivity`의 임시 검증 배선이다(`RoomFormLauncher` 참조처 실측). 이 표는 그대로 유효하다.
...
열린 것은 **D·G·H 셋이다.** D(서버 계약)와 H(세션 선행 조건)는 이 계획이 닫을 수 없고, G는 조건이 올 때까지 닫지 않는다. C·E·F는 닫힌 이력을 남겨 둔다. **3.0.0이 연 항목은 없다** — R-031·R-032·R-034는 셋 다 실측으로 확정된 것이라 미결로 남기지 않는다.

**H는 구현이 아니라 검증을 막는다.** 이 계획의 코드는 H와 무관하게 전부 만들어지고 단위 테스트로 검증되지만, 실기기 시나리오는 H가 닫히기 전까지 돌지 않는다([quickstart.md](./quickstart.md) §1).

| # | 무엇 | 닫는 조건 |
|---|---|---|
| D | **어긋남이 하나로 줄었다** — `description.maxLength`가 협의(30자)와 달리 여전히 `20`이다. 색 관련 두 건(`maxLength: 7` · `enum` 부재)은 2026-08-28T00:55:30 조회에서 **해소를 확인했다**([research.md](./research.md) R-030). **08-28T11:39:53 재조회에서도 `20` 그대로다**(R-033) | 서버가 `description.maxLength`를 30으로 고치면 닫힌다. 그때 [contracts/room-api.md](./contracts/room-api.md) §1의 인용을 재대조하며, 반영 여부는 [quickstart.md](./quickstart.md) S-9의 4번이 확인한다. **이 계획의 코드는 더 바뀌지 않는다** |
