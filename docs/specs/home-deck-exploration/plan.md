...
2. **정렬 3종 후보와 장소분류 라벨은 mock DataSource로 진행한다**(R-001·R-002). 배포 OpenAPI 문서에 **대응 API가 없다**(대조 근거는 [`contracts/deck-api.md`](./contracts/deck-api.md) §3). `group-room-form`이 이미 쓴 패턴(인터페이스 + mock 바인딩 + 계약 문서)을 따라 실서버 전환 비용을 `@Binds` 한 줄로 고정한다.
...
**저장소**: DataStore (마지막으로 보던 방 · 가이드 닫은 이력 **둘만**, R-004). 그 밖의 진행 상태는 화면 상태로만 존재한다.
...
**제약 조건**: 덱 전환 판정은 **전환이 일어나는 시점에 매번** 수행한다(FR-011) — 방 진입 시 계산해 두고 재사용하지 않는다. 빈 덱을 노출하지 않는다(EC-013).

**규모/범위**: 화면 1장(홈 탭) + 오버레이 3종(방 시트·액션 메뉴·가이드) + 상태 4종(덱·완료·빈 상태·로딩). FR 21건 · UX 4건 · TS 31건 · EC 15건.

**참조 API 문서**: `https://api.gguk.org/api-docs-json` · 조회 시점 **2026-08-26T21:33:25+09:00** · Team MINO API 1.0.0 · 오퍼레이션 21개. 대조 결과와 미구현 항목은 [`contracts/deck-api.md`](./contracts/deck-api.md)가 소유한다.
...
│   └── PlaceCardResponse.kt             # 실서버 전환 지점 ③
├── repository/
...
> mock DataSource 채택(R-001)은 위반이 아니라 **`group-room-form`이 이미 쓴 기존 패턴의 재적용**이다. 새 구조를 들이지 않으며, 실서버가 붙으면 `@Binds` 인자 타입 하나로 교체된다.
