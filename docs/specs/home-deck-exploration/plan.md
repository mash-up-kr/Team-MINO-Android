...
| **방 전환 툴팁 위치** | 시안 `2809:143382` | `MinoTooltip(position = Left, align = Center)` + 조립부 오프셋 — 화살표는 그 반대편 오른쪽 변에 붙는다 (R-016) |
| 방 선택 시트의 방 썸네일 | 시안 `4306:63731` | **변경 없음** — 기존 `room_thumbnail_*` 에셋과 같은 그림임을 대조로 확인 |
| 서버 호출 계약 중복 제거 — `다른 방 저장` | 설계 중 발견 | `savePinToRoom`을 걷고 `PlaceRepository.duplicatePin`을 쓴다 (R-019) |
| **「경과일 초기화 확인」 기록 소유 이관** | spec 4.0.0 FR-007·023 | `recordPlaceOpened`를 걷어내고 **홈은 부르지 않는다** — 기록은 [SCR-006]이 소유한다(R-019 갱신) |
