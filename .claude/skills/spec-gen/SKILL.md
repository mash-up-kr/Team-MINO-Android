...
> **산출물 위치**: 모든 산출물은 `docs/specs/{슬러그}/` 아래에 둔다. 이하 이 디렉터리를 `{작업 디렉터리}`라 부른다. 대시보드는 spec.md 첫 줄 `<!-- feature: {슬러그} -->` 주석으로 PR 커밋 경로를 만들므로, 디렉터리 이름은 그 슬러그와 같아야 하고 번호 접두어를 붙이지 않는다.
>
...
3. **슬러그 + 작업 디렉터리 결정**:
   - 슬러그는 kebab-case(영문 소문자/숫자/하이픈만), 기능을 대표하는 유니크한 이름. Figma·기획서에서 **자동 판단**한다. spec.md 첫 줄 주석과 디렉터리 이름에 같은 값을 쓴다.
   - `{작업 디렉터리} = docs/specs/{슬러그}/` (예: `docs/specs/mypage/`). 이 디렉터리와 그 하위 `assets/`를 만든다.
   - 같은 이름의 디렉터리가 이미 있으면 새로 만들지 말고 멈춘다 — 개정인지 슬러그 충돌인지 사용자에게 확인한다.
...
- 산출물은 `{작업 디렉터리}/spec.md` + `{작업 디렉터리}/assets/*.png`다 (예: `docs/specs/mypage/`). 사용자는 이를 **spec-center 대시보드에 drag-drop 업로드**한다 (대시보드의 붙여넣기 구조 검증 → 디자이너 컨펌 게이트로 이어짐). 스킬은 대시보드/Firestore에 직접 쓰지 않는다.
