...
- 작업 **시작 직후**와 **종료 직후** 각각 한 줄을 append 한다. 상태 칸은 `start` 그리고 보고와 같은 `done`·`blocked`·`failed`다:

  ```sh
  echo "$(date +%H:%M:%S) core-layer-developer T012 start | - | -" >> "$PROGRESS_LOG"
...
- Gradle 빌드처럼 오래 걸리는 단계에 들어가면 `note` 상태로 한 줄 남긴다. 그 밖의 중간 경과는 남기지 않는다 — 한 줄이 사용자에게 알림 하나다
- 이 로그는 **보고를 대체하지 않는다.** 최종 반환은 위 보고 형식 그대로 한다
- `context-analyst`는 쓰기 도구가 없어 로그를 남기지 않는다. 배정·수신 시점을 리드가 대신 찍는다
...
5. 작업 계획에 따라 구현을 실행한다:
   ...
   - **단계별 실행**: 다음 단계로 넘어가기 전에 각 단계를 완료한다
   - **의존성 준수**: 순차 작업은 순서대로 실행하고, 병렬 작업 [P]은 함께 실행할 수 있다
   - **TDD 방식 준수**: 대응되는 구현 작업보다 테스트 작업을 먼저 실행한다
   - **파일 기반 조정**: 같은 파일에 영향을 주는 작업은 순차적으로 실행해야 한다
   - **검증 체크포인트**: 진행하기 전에 각 단계의 완료 여부를 확인한다
   ...
   **동시 배정 규칙**:
   - `[P]` 마커가 있고 **파일 경로가 겹치지 않을 때만** 동시에 띄운다. tasks.md가 `[P]`를 붙였더라도 경로가 겹치면 한 전문가에게 순차 배정한다
   - 같은 모듈의 작업은 같은 전문가에게 몰아 배정한다 — 소유가 갈리면 서로의 산출물을 덮어쓴다
   - 한 번에 띄우는 전문가는 최대 3종으로 제한한다. 리드가 보고를 대조할 수 있는 한도다
...
7. 완료 검증:
   ...
   - 감사관이 지적한 위반은 리드가 수정 작업으로 만들어 소유 전문가에게 재배정한다. 남는 위반이 있으면 완료 보고에 명시한다
   - 재배정까지 모두 끝난 뒤 진행 로그 Monitor를 `TaskStop`으로 끈다. 로그 파일은 완료 보고 후 남겨두지 않는다
