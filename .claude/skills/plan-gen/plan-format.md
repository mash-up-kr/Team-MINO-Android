...
- **feature 모듈은 `core:data`를 직접 의존하지 않는다** — `core:domain` 인터페이스만 안다. DI 바인딩 소유 규칙은 [`dependency-injection.md`](../../../docs/conventions/dependency-injection.md)를 따른다.
...
> 의존 규칙: feature→core:navigation(전환 계약), feature→core:domain(인터페이스). DI 바인딩 소유는 `dependency-injection.md` 참조. feature 간 의존은 탭 셸→탭 feature만 예외
