...
> **산출물 위치**: `{작업 디렉터리}/plan.md` (컨펌된 spec.md가 들어 있는 `docs/specs/{슬러그}/`, spec.md와 형제).
>
...
2. **슬러그·작업 디렉터리·spec 버전 확정**: spec 첫 줄 `<!-- feature: {슬러그} -->`에서 슬러그를 읽어 `docs/specs/{슬러그}/`를 찾는다(없으면 사용자에게 위치 확인). spec `변경 이력` 최신 행에서 버전을 파싱한다.
...
- 산출물은 `{작업 디렉터리}/plan.md`다 (컨펌된 spec.md와 형제, 예: `docs/specs/mypage/plan.md`). 로컬 산출용일 뿐 레포에 직접 커밋하지 않는다 — 사용자가 **spec-center 대시보드에 업로드**하고 (spec_approved 이후), PR 단계에서 커밋된다. plan은 별도 컨펌 게이트 없이 PR 리뷰(얼라인)에서 검증된다.
