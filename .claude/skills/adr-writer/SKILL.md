...
1. **같은 폴더의 [`adr-format.md`](adr-format.md)를 먼저 Read**해 파일명·상태 값·템플릿 형식을 확인한다. 이 Skill 안에서 형식을 재정의하지 않는다.
2. 다음 번호를 정한다: `docs/adr/` 내 가장 큰 번호에 +1.
3. 대화 맥락에서 아래를 추출한다.
...
4. `adr-format.md`의 템플릿에 맞춰 새 ADR 파일을 작성한다. 상태는 기본 `Accepted`(사용자가 확정한 결정이므로) — 아직 논의 중이면 `Proposed`.
5. `docs/adr/README.md` 인덱스 표에 새 줄을 추가한다.
6. 이 결정이 기존 ADR을 대체하는 것이라면, `adr-format.md`의 상태 규칙에 따라 기존 ADR의 상태를 갱신한다(README 인덱스의 상태 셀도 함께).
...
- 산출물: 새 ADR 파일 + `docs/adr/README.md` 인덱스 갱신, 필요 시 대체된 기존 ADR의 상태 갱신.
