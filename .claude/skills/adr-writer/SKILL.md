...
3. `adr-format.md`의 템플릿에 맞춰 새 ADR 파일을 작성한다. 상태는 기본 `Accepted`(사용자가 확정한 결정이므로) — 아직 논의 중이면 `Proposed`.
4. `docs/adr/README.md` 인덱스 표에 새 줄을 추가한다.
5. 이 결정이 기존 ADR을 대체하는 것이라면, `adr-format.md`의 상태 규칙에 따라 기존 ADR의 상태를 갱신한다(README 인덱스의 상태 셀도 함께).
