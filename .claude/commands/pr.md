...
PR 생성 완료 직후, AskUserQuestion으로 사용자에게 어떤 `D-n` 라벨을 부착할지 묻고 선택된 라벨을 PR에 부착한다. `naver/d-day-labeler` Action이 이후 매일 카운트다운한다.

**라벨 규격·기본값·카운트다운 정책은 [`docs/conventions/pull-request.md`](../../docs/conventions/pull-request.md)의 "D-n 라벨 정책" 섹션을 단일 출처로 한다.** 커맨드 실행 시 해당 섹션을 Read하여 사용 라벨 목록을 확보한다.

### 9-1. 라벨 선택

위 정책의 사용 라벨 + "붙이지 않음" 옵션으로 AskUserQuestion을 구성하되, 기본값(`D-2`)에 "(Recommended)"를 표기한다. "붙이지 않음" 선택 시 이 PR은 카운트다운 대상에서 제외됨을 옵션 설명에 명시.

### 9-2. 라벨 부착

"붙이지 않음"이 아닌 경우 선택된 라벨을 부착:
```sh
gh pr edit "$PR_NUMBER" --add-label "<D-n>"
```

라벨이 레포에 정의되어 있지 않다는 에러가 나면 안내 후 건너뜀:
> ⚠️ `D-n` 라벨이 이 레포에 정의되어 있지 않습니다. 관리자에게 라벨 생성을 요청해주세요.

---

## 10. 코드 리뷰 코멘트 작성 (작성자 관점의 보충 설명)
...
- PR 생성 직후 **D-n 라벨을 사용자에게 물어 부착**한다(9단계). AskUserQuestion으로 `D-1`/`D-2`/`D-3`/`D-4`/`붙이지 않음` 중 하나를 선택받고, `D-2`를 기본 추천으로 표기한다. 부착 이후 매일 카운트다운은 `.github/workflows/d-day-labeler.yml`이 담당.
- PR 생성 직후 **작성자 관점의 인라인 Review Comment**를 10단계에서 작성한다. 리뷰어 톤(지적)이 아니라 작성자 톤(의도 설명)이다. 자명한 변경·본문 중복 설명에는 코멘트하지 않는다.
