...
> 주의: simplify가 의도한 기능 범위 밖의 변경을 만들 수 있다. 이후 논리 단위 분석 단계에서 해당 수정을 **별도 커밋**(`refactor:` 또는 `chore:`)으로 분리할지 판단한다. 원래 기능 변경과 단순 정리는 섞지 않는 것을 우선한다.

---

## 3. ktlint 게이트 (필수)

`simplify` 반영 직후, 논리 단위 분석 전에 **staged 코드의 포맷을 ktlint로 검증**한다. 위반이 남아있으면 커밋을 진행하지 않는다. **건너뛸 수 없다.**
...
### 3-2. 위반 처리
- **통과(BUILD SUCCESSFUL)**: 다음 단계로 진행.
- **위반 발생**: 위반 내용을 그대로 출력하고 **커밋을 진행하지 않는다**. 사용자에게 수동 수정 후 `/done` 재실행을 안내한다. 이 게이트는 **검증(`ktlintCheck`)만 수행**하며, `ktlintFormat` 등 자동 수정은 실행하지 않는다(포맷 수정은 개발자가 직접 판단·반영).

> ktlint 규칙·`.editorconfig` 컨벤션은 프로젝트 루트의 [`.editorconfig`](../../.editorconfig)를 단일 출처로 한다. 이 커맨드에서 규칙을 재정의하지 않는다.
...
- **`simplify` Skill 실행(2단계) → ktlint 게이트(3단계) → 논리 단위 분석(4단계)** 순서를 지킨다. simplify·ktlint 게이트는 모두 **건너뛰기 불가**. simplify로 인한 수정은 기능 변경과 섞지 않고 별도 커밋(`refactor:`/`chore:`)으로 분리하는 것을 우선.
- **ktlint 게이트(3단계)에서 `./gradlew ktlintCheck`로 staged 포맷을 검증**한다. 검증만 수행하며 `ktlintFormat` 등 자동 수정은 실행하지 않는다. 위반이 있으면 **커밋을 진행하지 않고** 사용자에게 수동 수정 후 재실행을 안내한다. 규칙은 [`.editorconfig`](../../.editorconfig)를 단일 출처로 한다.
