...
- 최종 포맷: `<prefix>/<issue-number>-<slug>/base` — 이슈 생성 시 만드는 브랜치는 항상 `/base` 접미사를 붙인다. 이유·개념은 [`base-branch.md`](../../docs/conventions/base-branch.md) 참조.
...
- 브랜치 이름은 **별도 승인 없이** Claude가 규칙에 맞게 구성하여 바로 체크아웃한다. 최종 이름은 항상 `/base` 접미사로 끝난다(`<prefix>/<issue-number>-<slug>/base`) — 이유·개념은 [`base-branch.md`](../../docs/conventions/base-branch.md) 참조.
