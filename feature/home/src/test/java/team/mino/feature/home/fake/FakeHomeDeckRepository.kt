 ...
 *
 * **[deckRequests]가 호출을 세는 이유** — 현재 방을 다시 골라도 덱을 다시 구성하지 않는다(EC-014). 상태가
 * 그대로인 것만으로는 "다시 받아 같은 값이 된 것"과 구별되지 않아 **요청 횟수**가 있어야 한다. 정렬·좌표도
 * 함께 남는다.
 *
 * **`savePinToRoom`은 여기 없다** — 이 인터페이스에서 빠졌다(R-019). 같은 동작은
 * [PlaceRepository.duplicatePin]이 대신하고, 그 더블은 `FakePlaceRepository`다
 * (`contracts/home-ui.md` §4.2.1).
 *
 * **「경과일 초기화 확인」을 세는 스위치도 없다.** spec 4.0.0에서 그 기록의 소유가 [SCR-006]으로 넘어가
 * 홈은 어느 저장소로도 부르지 않는다 — 셀 호출 자체가 없다.
 *
 * **실패 스위치는 둘이다.** 둘 다 계약이 실패를 [MinoDomainException]으로 던지기로 돼 있고, 화면의 주 데이터
 * (방 목록·덱) 로드라 실패가 스낵바가 아니라 상태에 담겨야 한다(`docs/conventions/error_handling.md` §5 1행).
 * 담기지 않으면 첫 로드가 실패한 화면이 로딩에서 걷히지 않는다 — 그 고착을 잡는 그물이 [getDeckFailure]·
 * [getRoomSummariesFailure]다. 기본값은 **「실패하지 않음」**(`null`)이다.
 */
