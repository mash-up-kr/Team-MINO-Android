 ...
 *
 * [TBD] `place.thumbnailUrl`이 없으면 [mockPlaceThumbnailUrl] 목업 사진으로 채운다 — 서버가 아직
 * 장소 사진을 안 내려주는 지금 상태에서 폴백 글리프만 계속 보이면 카드형이 텅 비어 보인다는 실기기
 * 확인 결과에 따른 임시 결정이다. 서버가 실제 사진 필드를 내려주기 시작하면 이 자리는 지운다.
 */
@Composable
internal fun PlaceListItem(
    ...
    MinoCardLocationList(
        ...
        modifier = modifier,
        thumbnailUrl = place.thumbnailUrl ?: mockPlaceThumbnailUrl(),
