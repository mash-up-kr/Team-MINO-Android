...
/**
 * 장소(핀) 단위 동작 계약 — 방 단위 동작은 [RoomRepository] 소관이다.
 *
 * `observePlaces`·`sharePlaces`는 배포된 서버 API로 근거를 대조했다(`docs/specs/room-detail/research.md`
 * D14). `deletePlace`는 대응 엔드포인트가 아직 없어 구현 시점 임시 처리로 남는다.
 */
