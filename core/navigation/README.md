...
- 인프라(`activity`·`screen`)는 두 전환 축 중 **어디에 공통으로 쓰이는가**로 패키지를 고른다.
...
이 모듈이 끌어오는 주요 라이브러리: `androidx-navigation-compose`, `androidx-activity-compose`, `kotlinx-serialization-json` (모두 `implementation`이라 전이되지 않음). 주로 Activity·`NavHost`·`Launcher`를 구현하는 feature 모듈이 의존한다.
