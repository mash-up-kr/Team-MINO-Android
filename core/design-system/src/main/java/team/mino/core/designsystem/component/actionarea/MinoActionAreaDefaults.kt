...
object MinoActionAreaDefaults {
    /**
     * `sticky = true`이거나 `extra`가 붙었을 때만 깔리는 배경. 콘텐츠 위에 떠 있는 표면이라
     * Normal이 아니라 Elevated를 쓴다. sticky 상단 페이드도 이 색이 투명에서 차오르는 형태다.
     */
    val surfaceColor: Color
        @Composable @ReadOnlyComposable get() = ActionAreaTokens.SurfaceColor.value
}
