...
internal const val MIXED_LABEL_DECK_BODY =
    """{"data":[
...
internal const val SHORT_DECK_BODY =
    """{"data":[
...
internal const val EMPTY_DECK_BODY = """{"data":[]}"""
 ...
 */
internal const val ALL_WORTH_VISITING_DECK_BODY =
    """{"data":[
 ...
 */
internal const val MISSING_FIELDS_DECK_BODY = """{"data":[{"images":["https://img/1"]}]}"""

/**
 * 아바타 객체가 색 없이 오는 응답.
 *
 * `/cards`의 OpenAPI 문서는 아바타를 `{ id: integer }`로 적어 두었지만 실제 응답에는 그 필드가 없다.
 * 문서를 그대로 믿고 `id`를 필수로 두었더니 덱 조회 전체가 `JsonConvertException`으로 죽었다 — 그 회귀를
 * 잡는 자리다.
 */
internal const val AVATAR_WITHOUT_COLOR_DECK_BODY =
    """{"data":[{"id":"pin-1","place":{"name":"연남동 감자탕","address":"서울 마포구 연남로 21"},
